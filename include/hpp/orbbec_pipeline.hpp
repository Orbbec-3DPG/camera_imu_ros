#ifndef ORBBEC_PIPELINE_HPP
#define ORBBEC_PIPELINE_HPP

#include "orbbec_types.hpp"
#include "orbbec_frame.hpp"
#include "orbbec_context.hpp"
...
namespace orbbec
{
    ...
    */
    ...
    public:
        ...
        */
        std::vector<stream_profile> get_streams() const
        {
            ...
            std::shared_ptr<orbbec_stream_profile_list> list(
                orbbec_pipeline_profile_get_streams(_pipeline_profile.get(), &e),
        ...
        */
        ...
        stream_profile get_stream(orbbec_stream stream_type, int stream_index = -1) const
        {
        ...
        */
        device get_device() const
        {
            ...
            std::shared_ptr<orbbec_device> dev(
                orbbec_pipeline_profile_get_device(_pipeline_profile.get(), &e),
    ...
    private:
        pipeline_profile(std::shared_ptr<orbbec_pipeline_profile> profile) :
            _pipeline_profile(profile)
        {

        }
        std::shared_ptr<orbbec_pipeline_profile> _pipeline_profile;
        friend class config;
    ...
    */
    ...
    public:
        ...
        {
            orbbec_error* e = nullptr;
            _config = std::shared_ptr<orbbec_config>(
                orbbec_create_config(&e),
                orbbec_delete_config);
            error::handle(e);
        }
        ...
        * \param[in] format         Stream data format - pixel format for images streams, of data type for other streams. ORBBEC_FORMAT_ANY indicates any.
        * \param[in] framerate      Stream frames per second. 0 indicates any.
        */
        void enable_stream(orbbec_stream stream_type, int stream_index, int width, int height, orbbec_format format = ORBBEC_FORMAT_ANY, int framerate = 0)
        {
            orbbec_error* e = nullptr;
            orbbec_config_enable_stream(_config.get(), stream_type, stream_index, width, height, format, framerate, &e);
            error::handle(e);
        }

        //Stream type and possibly also stream index
        void enable_stream(orbbec_stream stream_type, int stream_index = -1)
        {
            enable_stream(stream_type, stream_index, 0, 0, ORBBEC_FORMAT_ANY, 0);
        }

        //Stream type and resolution, and possibly format and frame rate
        void enable_stream(orbbec_stream stream_type, int width, int height, orbbec_format format = ORBBEC_FORMAT_ANY, int framerate = 0)
        {
            enable_stream(stream_type, -1, width, height, format, framerate);
        }

        //Stream type and format
        void enable_stream(orbbec_stream stream_type, orbbec_format format, int framerate = 0)
        {
            enable_stream(stream_type, -1, 0, 0, format, framerate);
        }

        //Stream type and format
        void enable_stream(orbbec_stream stream_type, int stream_index, orbbec_format format, int framerate = 0)
        {
        ...
        */
        ...
        {
            orbbec_error* e = nullptr;
            orbbec_config_enable_all_stream(_config.get(), &e);
            error::handle(e);
        }
        ...
        *
        * \param[in] serial device serial number, as returned by ORBBEC_CAMERA_INFO_SERIAL_NUMBER
        */
        void enable_device(const std::string& serial)
        {
            orbbec_error* e = nullptr;
            orbbec_config_enable_device(_config.get(), serial.c_str(), &e);
            error::handle(e);
        }
        ...
        */
        ...
        {
            orbbec_error* e = nullptr;
            orbbec_config_enable_device_from_file_repeat_option(_config.get(), file_name.c_str(), repeat_playback, &e);
            error::handle(e);
        }
        ...
        */
        ...
        {
            orbbec_error* e = nullptr;
            orbbec_config_enable_record_to_file(_config.get(), file_name.c_str(), &e);
            error::handle(e);
        }
        ...
        */
        void disable_stream(orbbec_stream stream, int index = -1)
        {
            orbbec_error* e = nullptr;
            orbbec_config_disable_indexed_stream(_config.get(), stream, index, &e);
            error::handle(e);
        }
        ...
        */
        ...
        {
            orbbec_error* e = nullptr;
            orbbec_config_disable_all_streams(_config.get(), &e);
            error::handle(e);
        }
        ...
        */
        pipeline_profile resolve(std::shared_ptr<orbbec_pipeline> p) const
        {
            orbbec_error* e = nullptr;
            auto profile = std::shared_ptr<orbbec_pipeline_profile>(
                orbbec_config_resolve(_config.get(), p.get(), &e),
                orbbec_delete_pipeline_profile);
        ...
        */
        bool can_resolve(std::shared_ptr<orbbec_pipeline> p) const
        {
            orbbec_error* e = nullptr;
            int res = orbbec_config_can_resolve(_config.get(), p.get(), &e);
            error::handle(e);
            return res != 0;
        }

        std::shared_ptr<orbbec_config> get() const
        {
            return _config;
        }
    private:
        config(std::shared_ptr<orbbec_config> config) : _config(config)
        {
        }
        std::shared_ptr<orbbec_config> _config;
    ...
    */
    ...
    public:
        ...
        */
        ...
        {
            orbbec_error* e = nullptr;
            _pipeline = std::shared_ptr<orbbec_pipeline>(
                orbbec_create_pipeline(ctx._context.get(), &e),
                orbbec_delete_pipeline);
            error::handle(e);
        }
        ...
        */
        ...
        {
            orbbec_error* e = nullptr;
            auto p = std::shared_ptr<orbbec_pipeline_profile>(
                orbbec_pipeline_start(_pipeline.get(), &e),
                orbbec_delete_pipeline_profile);
        ...
        */
        pipeline_profile start(const config& config)
        {
            orbbec_error* e = nullptr;
            auto p = std::shared_ptr<orbbec_pipeline_profile>(
                orbbec_pipeline_start_with_config(_pipeline.get(), config.get().get(), &e),
                orbbec_delete_pipeline_profile);
        ...
        */
        ...
        {
            orbbec_error* e = nullptr;
            orbbec_pipeline_stop(_pipeline.get(), &e);
            error::handle(e);
        }
        ...
        */
        ...
        {
            orbbec_error* e = nullptr;
            frame f(orbbec_pipeline_wait_for_frames(_pipeline.get(), timeout_ms, &e));
            error::handle(e);
        ...
        */
        bool poll_for_frames(frameset* f) const
        {
            ...
            orbbec_error* e = nullptr;
            orbbec_frame* frame_ref = nullptr;
            auto res = orbbec_pipeline_poll_for_frames(_pipeline.get(), &frame_ref, &e);
            error::handle(e);
        ...
        */
        pipeline_profile get_active_profile() const
        {
            orbbec_error* e = nullptr;
            auto p = std::shared_ptr<orbbec_pipeline_profile>(
                orbbec_pipeline_get_active_profile(_pipeline.get(), &e),
                orbbec_delete_pipeline_profile);
        ...
        operator std::shared_ptr<orbbec_pipeline>() const
        {
            return _pipeline;
        }
    ...
    private:
        context _ctx;
        std::shared_ptr<orbbec_pipeline> _pipeline;
        friend class config;
    };
}
#endif // ORBBEC_PIPELINE_HPP
