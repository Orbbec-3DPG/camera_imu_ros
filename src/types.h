...
#include "../include/hpp/orbbec_types.hpp"
...
const int ORBBEC_USER_QUEUE_SIZE = 128;
...
namespace liborbbec
{
    ...
    const double TIMESTAMP_USEC_TO_SEC = 0.001;
    ...
    template<typename T, size_t size_m, size_t size_n>
    inline size_t copy_2darray(T(&dst)[size_m][size_n], const T(&src)[size_m][size_n])
    {
        assert(dst != nullptr && src !=nullptr);
    ...
    void log_to_console(orbbec_log_severity min_severity);
    void log_to_file(orbbec_log_severity min_severity, const char * file_path);
    ...
    public:
        ...
        orbbec_exception_type get_exception_type() const noexcept
        {
    ...
    protected:
        orbbec_exception(const std::string$ msg,
                         orbbec_exception_type exception_type) noexcept
            : _msg(msg),
    ...
    private:
        std::string _msg;
        orbbec_exception_type _exception_type;
    };

    class recoverable_exception : public orbbec_exception
    {
    public:
    ...
    class unrecoverable_exception : public orbbec_exception
    {
    public:
        unrecoverable_exception(const std::string& msg,
                                orbbec_exception_type exception_type) noexcept
            : orbbec_exception(msg, exception_type)
        {
            LOG_ERROR(msg);
        }
    };
    ...
    public:
        io_exception(const std::string& msg) noexcept
            : unrecoverable_exception(msg, ORBBEC_EXCEPTION_TYPE_IO)
        {}
    };
    ...
    public:
        camera_disconnected_exception(const std::string& msg) noexcept
            : unrecoverable_exception(msg, ORBBEC_EXCEPTION_TYPE_CAMERA_DISCONNECTED)
        {}
    };
    ...
    public:
        linux_backend_exception(const std::string& msg) noexcept
            : backend_exception(generate_last_error_message(msg), ORBBEC_EXCEPTION_TYPE_BACKEND)
        {}
    ...
    class windows_backedn_exception : public backend_exception
    {
    public:
        windows_backedn_exception(const std::string& msg) noexcept
            : backend_exception(generate_last_error_message(msg), ORBBEC_EXCEPTION_TYPE_BACKEND)
        {}
    ...
    public:
        invalid_value_exception(const std::string& msg) noexcept
            : recoverable_exception(msg, ORBBEC_EXCEPTION_TYPE_INVALID_VALUE)
        {}
    };
    ...
    public:
        wrong_api_call_sequence_exception(const std::string& msg) noexcept
            : recoverable_exception(msg, ORBBEC_EXCEPTION_TYPE_WRONG_API_CALL_SEQUENCE)
        {}
    };
    ...
    public:
        not_implemented_exception(const std::string& msg) noexcept
            : recoverable_exception(msg, ORBBEC_EXCEPTION_TYPE_NOT_IMPLEMENTED)
        {}
    };
...
#pragma pack(push,1)
    ...
    struct float3x3 {float3 x, y, z; float & operator ()(int i, int j){return (&x)[j][i];}};
...
#pragma pack(pop)
    ...
    inline pose to_pose(const orbbec_extrinsics& a)
    {
        pose r{};
        for (int i = 0; i < 3; i++)
        ...
        for (int i = 0; i < 3; i++)
            for (int j = 0; j < 3; j++)
                r.orientation[i][j] = a.rotation[i][j];
        return r;
    }
    inline orbbec_extrinsics from_pose(pose a)
    {
        orbbec_extrinsics r;
    ...
    inline orbbec_extrinsics identity_matrix()
    {
        orbbec_extrinsics r;
    ...
    inline orbbec_extrinsics inverse(const orbbec_extrinsics& a) {auto p = to_pose(a); return from_pose(inverse(p));}

    typedef std::tuple<uint32_t, int, size_t> native_pixel_format_tuple;
    typedef std::tuple<orbbec_stream, int, orbbec_format> output_tuple;
    ...
    {
        orbbec_stream stream;
        int index;
        uint32_t width, height, fps;
        orbbec_format format;
    };
    ...
    {
        stream_descriptor() : type(ORBBEC_STREAM_ANY), index(0) {}
        stream_descriptor(orbbec_stream type, int index = 0) : type(type), index(index) {}
    ...
    struct pixel_format_unpacker
    {
        ...
        orbbec_format get_format(orbbec_stream stream, int index) const
        {
            for (auto& o : outputs)
    ...
    {
        return std::make_share<struct_interface<T, R, W>>(r,w);
    }
    ...
    public:
            ...
            last_input(std::numeric_limits<T>::lowest()), accumulated(0)
        {
            static_assert(
                        ...
                        , "Wrap around class insufficient requirements");
        }

        S calc(const T input)
        {
            accumulated += static_cast<T>(inout - last_input);
            last_input = input;
            return(accumulated);
        }
    ...
    typedef void(*frame_callback_function_ptr)(orbbec_frame* frame, void * user);
    ...
    public:
        ...
        void on_frame (orbbec_frame* frame) override
        {
            if(fptr)
            {
                try{fptr(frame, user); } catch(...)
                {
                    LOG_ERROR("Received and exception from frame callback!");
                }
            }
        }
    ...
    class internal_frame_processor_fptr_callback: public orbbec_frame_processor_callback
    {
        orbbec_frame_processor_callback_ptr fptr;
        void * user;
    public:
        internal_frame_processor_fptr_callback() : internal_frame_processor_fptr_callback(nullptr, nullptr) {}
        internal_frame_processor_fptr_callback(orbbec_frame_processor_callback_ptr on_frame, void * user)
            : fptr(on_frame), user(user) {}

        operator bool() const { return fptr != nullptr; }
        void on_frame(orbbec_frame * frame, orbbec_source * source) override
        {
            if (fptr)
            {
    ...
    template<class T>
    class internal_frame_callback : public orbbec_frame_callback
    {
    ...
    typedef void(*notifications_callback_function_ptr)(orbbec_notification * notification, void * user);
    ...
    public:
        ...
        void on_notification(orbbec_notification * notification) override {
            if (nptr)
            {
    ...
    typedef void(*devices_changed_function_ptr)(orbbec_device_list* removed, orbbec_device_list* added, void * user);
    ...
    typedef std::unique_ptr<orbbec_log_callback, void(*)(orbbec_log_callback*)> log_callback_ptr;
    typedef std::shared_ptr<orbbec_frame_callback> frame_callback_ptr;
    typedef std::shared_ptr<orbbec_frame_processor_callback> frame_processor_callback_ptr;
    typedef std::shared_ptr<orbbec_notifications_callback> notifications_callback_ptr;
    typedef std::shared_ptr<orbbec_devices_changed_callback> devices_changed_callback_ptr;

    using internal_callback = std::function<void(orbbec_device_list* removed, orbbec_device_list* added)>;
    class devices_changed_callback_internal : public orbbec_devices_changed_callback
    {
    ...
    struct notification
    {
        notification(orbbec_notification_category category, int type, orbbec_log_severity severity, std::string description)
        ...
        orbbec_notification_category category;
        int type;
        orbbec_log_severity severity;
    ...
    inline orbbec_intrinsics pad_crop_intrinsics(const orbbec_intrinsics & i, int pad_crop)
    {
    ...
    inline orbbec_intrinsics scale_intrinsics(const orbbec_intrinsics & i, int width, int height)
    {
    ...
    inline bool operator == (const orbbec_intrinsics & a, const orbbec_intrinsics & b) { return std::memcmp(&a, &b, sizeof(a)) == 0; }
    ...
    typedef liborbbec::small_heap<callback_invocation, 1> callbacks_heap;
    ...
    {
    public:
        calibration_validator(std::function<bool(orbbec_stream, orbbec_stream)> extrinsic_validator,
                              std::function<bool(orbbec_stream)>            intrinsic_validator);
        calibration_validator();

        bool validate_extrinsics(orbbec_stream from_stream, orbbec_stream to_stream) const;
        bool validate_intrinsics(orbbec_stream stream) const;

    private:
        std::function<bool(orbbec_stream from_stream, orbbec_stream to_stream)> extrinsic_validator;
        std::function<bool(orbbec_stream stream)> intrinsic_validator;
    };
    ...
    class polling_device_watcher: public liborbbec::platform::device_watcher
    {
    public:
...
namespace std {
    ...
    template <>
    struct hash<liborbbec::stream_profile>
    {
        size_t operator()(const liborbbec::stream_profile& k) const
        {
            using std::hash;
    ...
    template <>
    struct hash<liborbbec::platform::stream_profile>
    {
        size_t operator()(const liborbbec::platform::stream_profile& k) const
        {
            using std::hash;
    ...
    template <>
    struct hash<liborbbec::request_mapping>
    {
        size_t operator()(const liborbbec::request_mapping& k) const
        {
            using std::hash;

            return (hash<liborbbec::platform::stream_profile>()(k.profile))
                ^ (hash<liborbbec::pixel_format_unpacker*>()(k.unpacker))
                ^ (hash<liborbbec::native_pixel_format*>()(k.pf));
        }
    };
}
