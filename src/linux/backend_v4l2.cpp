...
namespace liborbbec
{
    namespace platform
    {
        ...
        std::string v4l_uvc_device::fourcc_to_string(uint32_t id) const
        {
            ...
            liborbbec::copy(fourcc_buff, &device_fourcc, sizeof(device_fourcc));
        ...
        void v4l_uvc_device::poll()
        {
                ...
                else if(FD_ISSET(_fd, &fds))
                {
                    ...
                    if (_is_started)
                    {
                                ...
                                buf.bytesused > 0)
                        {
                            ...
                            liborbbec::notification n = { ORBBEC_NOTIFICATION_CATEGORY_FRAME_CORRUPTED, 0, ORBBEC_LOG_SEVERITY_WARN, s.str()};
            ...
            else
            {
                ...
                liborbbec::notification n = {ORBBEC_NOTIFICATION_CATEGORY_FRAMES_TIMEOUT, 0, ORBBEC_LOG_SEVERITY_WARN,  "Frames didn't arrived within 5 seconds"};
        ...
        bool v4l_uvc_device::get_pu(orbbec_option opt, int32_t& value) const
        {
            ...
            if (ORBBEC_OPTION_ENABLE_AUTO_EXPOSURE==opt)  { control.value = (V4L2_EXPOSURE_MANUAL==control.value) ? 0 : 1; }
            value = control.value;

            return true;
        }

        bool v4l_uvc_device::set_pu(orbbec_option opt, int32_t value)
        {
            struct v4l2_control control = {get_cid(opt), value};
            if (ORBBEC_OPTION_ENABLE_AUTO_EXPOSURE==opt) { control.value = value ? V4L2_EXPOSURE_APERTURE_PRIORITY : V4L2_EXPOSURE_MANUAL; }
        ...
        control_range v4l_uvc_device::get_pu_range(orbbec_option option) const
        {
            // Auto controls range is trimed to {0,1} range
            if(option >= ORBBEC_OPTION_ENABLE_AUTO_EXPOSURE && option <= ORBBEC_OPTION_ENABLE_AUTO_WHITE_BALANCE)
            {
        ...
        uint32_t v4l_uvc_device::get_cid(orbbec_option option)
        {
            switch(option)
            {
            case ORBBEC_OPTION_BACKLIGHT_COMPENSATION: return V4L2_CID_BACKLIGHT_COMPENSATION;
            case ORBBEC_OPTION_BRIGHTNESS: return V4L2_CID_BRIGHTNESS;
            case ORBBEC_OPTION_CONTRAST: return V4L2_CID_CONTRAST;
            case ORBBEC_OPTION_EXPOSURE: return V4L2_CID_EXPOSURE_ABSOLUTE; // Is this actually valid? I'm getting a lot of VIDIOC error 22s...
            case ORBBEC_OPTION_GAIN: return V4L2_CID_GAIN;
            case ORBBEC_OPTION_GAMMA: return V4L2_CID_GAMMA;
            case ORBBEC_OPTION_HUE: return V4L2_CID_HUE;
            case ORBBEC_OPTION_SATURATION: return V4L2_CID_SATURATION;
            case ORBBEC_OPTION_SHARPNESS: return V4L2_CID_SHARPNESS;
            case ORBBEC_OPTION_WHITE_BALANCE: return V4L2_CID_WHITE_BALANCE_TEMPERATURE;
            case ORBBEC_OPTION_ENABLE_AUTO_EXPOSURE: return V4L2_CID_EXPOSURE_AUTO; // Automatic gain/exposure control
            case ORBBEC_OPTION_ENABLE_AUTO_WHITE_BALANCE: return V4L2_CID_AUTO_WHITE_BALANCE;
            case ORBBEC_OPTION_POWER_LINE_FREQUENCY : return V4L2_CID_POWER_LINE_FREQUENCY;
            case ORBBEC_OPTION_AUTO_EXPOSURE_PRIORITY: return V4L2_CID_EXPOSURE_AUTO_PRIORITY;
        ...
        void v4l_uvc_device::capture_loop()
        {
            ...
            catch (const std::exception& ex)
            {
                ...
                liborbbec::notification n = {ORBBEC_NOTIFICATION_CATEGORY_UNKNOWN_ERROR, 0, ORBBEC_LOG_SEVERITY_ERROR, ex.what()};
