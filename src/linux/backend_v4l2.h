...
namespace liborbbec
{
    namespace platform
    {
        ...
        {
        public:
            buffer(int fd, bool use_memoery_map, int index);
            void prepare_for_streaming(int id);
            ~buffer();
        ...
        public:
            ...
            ~v4l_usb_device();
            static void foreach_usb_device(libusb_context* usb_context,
                                           std::function<void(const usb_device_info&, libusb_device*)> action);
            std::vector<uint8_t> send_receive(const std::vector<uint8_t> &data,
                                              int timeout_ms = 5000,
        ...
        public:
            static void foreach_uvc_device(
                    std::function<void(const uvc_device_info&, const std::string&)> action);
            ...
            bool set_pu(orbbec_option opt, int32_t value) override;
            control_range get_pu_range(orbbec_option option) const override;
        ...
        private:
            static uint32_t get_cid(orbbec_option option);
            void capture_loop();
            bool has_metadata();
