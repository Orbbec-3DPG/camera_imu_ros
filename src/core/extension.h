...
/*! Preprocessor Macro to define mapping between orbbec_extension and their respective interface (and vice versa) */
#define MAP_EXTENSION(E, T)                                      \
    ...
    template<> struct TypeToExtension<T> {                       \
        static constexpr orbbec_extension value = E;                \
        static constexpr const char* to_string() { return #T; }; \
    }
...
namespace liborbbec
{
    ...
    {
    public:
        virtual bool extend_to(orbbec_extension extension_type, void** ptr) = 0;
    ...
    //Creating Helper functions to map orbbec_extension enums to actual interface
    template<orbbec_extension> struct ExtensionToType;
