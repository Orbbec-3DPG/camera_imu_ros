...
namespace liborbbec
{
    ...
    public:
        ...
        virtual orbbec_stream get_stream_type() const = 0;
        virtual void set_stream_type(orbbec_stream stream) = 0;
    };

    class stream_profile_interface : public stream_interface, public recordable<stream_profile_interface>
    {
    public:
        virtual orbbec_format get_format() const = 0;
        virtual void set_format(orbbec_format format) = 0;
        ...
        virtual orbbec_stream_profile* get_c_wrapper() const = 0;
        virtual void set_c_wrapper(orbbec_stream_profile* wrapper) = 0;
    };

    class frame_interface : public sensor_part
    {
    public:
        virtual orbbec_metadata_type get_frame_metadata(const orbbec_frame_metadata_value& frame_metadata) const = 0;
        virtual bool supports_frame_metadata(const orbbec_frame_metadata_value& frame_metadata) const = 0;
        virtual const byte* get_frame_data() const = 0;
        //TODO: add virtual uint64_t get_frame_data_size() const = 0;
        virtual orbbec_time_t get_frame_timestamp() const = 0;
        virtual orbbec_timestamp_domain get_frame_timestamp_domain() const = 0;
        virtual void set_timestamp(double new_ts) = 0;
        virtual unsigned long long get_frame_number() const = 0;

        virtual void set_timestamp_domain(orbbec_timestamp_domain timestamp_domain) = 0;
        virtual orbbec_time_t get_frame_system_time() const = 0;
        ...
        virtual orbbec_time_t get_frame_callback_start_time_point() const = 0;
        virtual void update_frame_callback_start_ts(orbbec_time_t ts) = 0;
        ...
        virtual void log_callback_start(orbbec_time_t timestamp) = 0;
        virtual void log_callback_end(orbbec_time_t timestamp) const = 0;
    ...
    public:
        ...
        virtual std::pair<uint32_t, orbbec_extrinsics> get_extrinsics(const stream_interface& stream) const = 0;
    ...
    MAP_EXTENSION(ORBBEC_EXTENSION_DEPTH_SENSOR, liborbbec::depth_sensor);
    ...
    MAP_EXTENSION(ORBBEC_EXTENSION_DEPTH_STEREO_SENSOR, liborbbec::depth_stereo_sensor);
