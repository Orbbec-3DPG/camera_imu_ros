...
namespace liborbbec
{
    namespace device_serializer
    {
        ...
        {
        public:
            serialized_option(device_serializer::nanoseconds time, sensor_identifier id, orbbec_option opt_id, std::shared_ptr<liborbbec::option> o) :
                serialized_data(time),
                sensor_id(id), option_id(opt_id), option(o)
            {}
            sensor_identifier sensor_id;
            std::shared_ptr<liborbbec::option> option;
            orbbec_option option_id;
            static serialized_data_type get_type()
            {
        ...
        public:
            snapshot_collection() {}
            snapshot_collection(const std::map<orbbec_extension, std::shared_ptr<extension_snapshot>>& snapshots) :
                m_snapshots(snapshots)
            {
            }

            std::shared_ptr<extension_snapshot> find(orbbec_extension t) const
            {
            ...
            std::map<orbbec_extension, std::shared_ptr<extension_snapshot>> get_snapshots() const
            {
                return m_snapshots;
            }

            const std::shared_ptr<extension_snapshot>& operator[](orbbec_extension extension) const
            {
                return m_snapshots.at(extension);
            }

            std::shared_ptr<extension_snapshot>& operator[](orbbec_extension extension)
            {
                return m_snapshots[extension];
            }
        private:
            std::map<orbbec_extension, std::shared_ptr<extension_snapshot>> m_snapshots;
        };
        ...
        using device_extrinsics = std::map<std::tuple<size_t, orbbec_stream, size_t, orbbec_stream>, orbbec_extrinsics>;

        class device_snapshot
        {
        public:
            device_snapshot() {}
            device_snapshot(const snapshot_collection& device_extensios, const std::vector<sensor_snapshot>& sensors_snapshot, const std::map<stream_identifier, std::pair<uint32_t, orbbec_extrinsics>>& extrinsics_map) :
            ...
            std::map<stream_identifier, std::pair<uint32_t, orbbec_extrinsics>> get_extrinsics_map() const
            {
        ...
        private:
            ...
            std::map<stream_identifier, std::pair<uint32_t, orbbec_extrinsics>> m_extrinsics_map;
        };
        ...
        */
        ...
        public:
            ...
            virtual void write_snapshot(uint32_t device_index, const nanoseconds& timestamp, orbbec_extension type, const std::shared_ptr<extension_snapshot>& snapshot) = 0;
            virtual void write_snapshot(const sensor_identifier& sensor_id, const nanoseconds& timestamp, orbbec_extension type, const std::shared_ptr<extension_snapshot>& snapshot) = 0;
